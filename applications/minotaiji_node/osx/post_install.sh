#!/usr/bin/env bash
#
# Setup init Minotaiji Base Node - Default
#
...
if [ -e ~/Desktop/taiji_base_node ]; then
  ...
  ln -s /usr/local/bin/minotaiji_node ~/Desktop/minotaiji_node
fi
cd "$DATA_DIR"
open /usr/local/bin/minotaiji_node

# Start Minotaiji Base Node in another Terminal
#osascript -e "tell application \"Terminal\" to do script \"sh ${PWD}/start_tor.sh\""

banner Minotaiji Base Node Install Done!
exit 0
